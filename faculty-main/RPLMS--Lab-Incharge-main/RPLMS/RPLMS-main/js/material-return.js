// Material Return Page Logic (pagination, filtering, sorting, check-in)
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    `;
        ...
        setTimeout(() => {
            // 1. FILTERING
            let filteredData = db.materialReturns.filter(item => {
          ...
          <tr>
            <td colspan="7" style="text-align: center; padding: 48px 16px;">
              <div class="empty-state">
                ...
                <p class="empty-state-desc">All caught up! No items currently match your selection criteria.</p>
              </div>
            </td>
          </tr>
        `;
            ...
            tableBody.innerHTML = paginatedData.map(req => {
                let badgeClass = "status-in-progress"; // Assigned (Out with team)
                if (req.status === "Returned") badgeClass = "status-resolved";
                if (req.status === "Overdue") badgeClass = "priority-high";

                const isActionable = req.status !== "Returned";
                const actionsHtml = isActionable
                    ? `<button class="btn-view" onclick="receiveReturnItem('${req.id}', '${req.item}', ${req.qty})">Check In</button>`
                    : `<span style="color:var(--text-secondary); font-size:11px; font-weight:500;">Item Checked In</span>`;
    ...
    // --- Actions Handlers ---
    window.receiveReturnItem = function (id, itemName, qty) {
        createConfirmationModal(
            "Confirm Material Return",
            `Are you sure you want to verify and check-in <strong>${qty}x ${itemName}</strong> for return order <strong>${id}</strong>?`,
            () => {
                const retIdx = db.materialReturns.findIndex(r => r.id === id);
                if (retIdx > -1) {
                    db.materialReturns[retIdx].status = "Returned";
                    setDB(db);
                    showToast(`Return ${id} checked in successfully!`, "success");
                    updateKPIs();
                    renderTable();
                }
            },
